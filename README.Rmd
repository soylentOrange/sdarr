...
`sdar()` and `sdar_lazy()` will create diagnostic plots throughout calculations, which will only be shown when requested (i.e. set `plot = TRUE` for obtaining a [crated](https://github.com/r-lib/carrier) plot-function of the final fit, or `plot.all = TRUE` for showing all additional diagnostic plots). To have a plot drawn later, you can call the provided plot-function from the results, when you set `plotFun = TRUE` (or `plotFun.all = TRUE`).  
