...
assemble_report <- function(normalized_data,
  ...
  if (showPlots || savePlots) {
    ...
    data.normalized.unnormalized <- normalized_data[["data.normalized"]] %>%
      dplyr::mutate("x.data" = .data$x.normalized * x.tangent + x.shift) %>%
      dplyr::mutate("y.data" = .data$y.normalized * y.tangent + y.shift) %>%
      dplyr::mutate("y.fit" = .data$x.data * finalSlope + trueIntercept) %>%
      dplyr::select(-c("otr.idx", "x.normalized", "y.normalized"))


    # generate function for plotting the Test Record with fit
    plot.fit <- carrier::crate(
      function(value) {
      ...
      plot.data = data.frame(
        "x.data" = data.normalized.unnormalized$x.data %>% unlist(TRUE, FALSE),
