...
find_linear_regressions.lazy <- function(data.normalized, nCandidates) {
  ...
  optimum.fits <- data.frame(round = round.idx,
                             count = round.count,
                             round.start = round.idx) %>%
    tidyr::nest(.by = round, .key = "limits") %>%
    dplyr::mutate(limits = purrr::map(limits, carrier::crate(\(limit) {
    ...
    dplyr::mutate(fit = furrr::future_map(bounds, carrier::crate(\(bound) {
      ...
      data.frame(norm.residual = norm_residual,
...
find_linear_regressions.subsampled <- function(data.normalized,
  ...
  best.fits <- seq.int(fit.rep) %>%
    ...
    check_fit_quality.lazy = check_fit_quality.lazy))) %>%
    dplyr::select(mc.idx, data.quality.check, fit) %>%
    tidyr::unnest(fit) %>%
