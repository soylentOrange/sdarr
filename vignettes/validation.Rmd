---
title: "Validation against Reference Data Set E3076-DS1(2018)"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Validation against Reference Data Set E3076-DS1(2018)}
...
sdarr.get_DS1.data <- carrier::crate(function(localFileName = NULL) {
  ...
  if(is.null(filename)) {
    # otherwise download file from the Internet 
    utils::download.file("https://web.archive.org/web/20190905162007/https://www.astm.org/COMMIT/E3076-DS1(2018).zip",
                         "E3076_DS1_2018.zip")
    
    # Extract the Zip Archive
    filename <- utils::unzip("E3076_DS1_2018.zip")
  }
  
  # read Data Set from unzipped file
  E3076_DS1_2018.rowwise <- readr::read_csv(filename,
                                            col_names = FALSE,
                                            trim_ws = FALSE,
  ...
  data.startIdx <- E3076_DS1_2018.rowwise %>%
    dplyr::mutate(data.start = stringr::str_detect(X1, stringr::coll("<Data>"))) %>%
    dplyr::filter(data.start) %>% {.$rownum %>% as.numeric}
  
  # find end of the data section 
  data.endIdx <- E3076_DS1_2018.rowwise %>%
    dplyr::mutate(data.end = stringr::str_detect(X1, stringr::coll("</Data>"))) %>%
    dplyr::filter(data.end) %>% {.$rownum %>% as.numeric}
  
  # read the names and units from test record
  E3076_DS1_2018.names <- readr::read_table(filename, 
                                      show_col_types = FALSE,
                                      skip = data.startIdx + 3, 
                                      n_max = 1)
  
  # read the data-points from the test record
  E3076_DS1_2018.data <- readr::read_table(filename,
                                           col_names = FALSE,
                                           show_col_types = FALSE,
...
# Get Data from Reference Data Set E3076-DS1(2018)
E3076_DS1_2018 <- if(!file.exists("E3076-DS1(2018).txt")) {
  sdarr.get_DS1.data()
} else {
  sdarr.get_DS1.data("E3076-DS1(2018).txt")
}
```
...
# plot of the data
E3076_DS1_2018 %>% {
  ggplot(data = ., aes(x = extensometer, y = force, color = "E3076_DS1_2018")) +
    geom_line() +
    theme_bw() +
  ...
  config(displaylogo = F, showSendToCloud = F)
```
...
# plot fit (with 95% CI) and lazy fit against original test record
{
  ...
  # add a (empty) data point at 0mm extension to see the difference in intercept
  plot.data <- data.frame(x.data = 0, y.data = NA) %>% 
    dplyr::bind_rows(plot.data) %>% 
    dplyr::bind_cols(y.data.prediction,
                     y.data.prediction.lazy)
  
  y.data.max <- plot.data$y.data %>% max(na.rm = T)
  ...
  ggplot(data = plot.data, 
    ...
    geom_line(data = plot.data %>% 
              ...
              aes(x = x.data, y = y.data, color = "fit-range (sdarr)")) +
    geom_line(data = plot.data %>% 
                dplyr::filter(dplyr::between(x.data, 
                                             x.lowerBound.lazy, x.upperBound.lazy)),
              aes(x = x.data, y = y.data, color = "fit-range (sdarr.lazy)")) +
    geom_line(data = plot.data %>% 
                dplyr::filter(fit <= y.data.max),
              aes(x = x.data, y = fit, color = "fit (sdarr)")) +
    geom_ribbon(data = plot.data %>% 
                dplyr::filter(fit <= y.data.max),
                aes(x = x.data, y = fit, ymin = lwr, ymax = upr, 
                    color = "fit (sdarr)"),
                alpha = 0.25) +
    geom_line(data = plot.data %>% 
    ...
    theme_bw() +
    labs(title = "Final fit (with 95% CI) vs. final lazy fit",
       x = plot.xlab,
       y = plot.ylab) 
  } %>% 
  ggplotly(dynamicTicks = T) %>%
...
Even though this is more a benchmarking than a *real* validation, the results of the sdarr-function are practically identical to the results listed in [ASTM E3076-18](https://doi.org/10.1520/E3076-18) and in [NIST Technical Note 2050 by E. Lucon](https://doi.org/10.6028/NIST.TN.2050). Considering the 95% confidence of the fitting results, same seems true for the random sub-sampling modification used in sdarr.lazy.
