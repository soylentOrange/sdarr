...
The final slope has a relative difference of `r paste0(round(((lazy_result$sdar$finalSlope %>% as.numeric()) - 328.1)/328.1*100, 2), " %")` to the reference value from ASTM E3076.  
  
However, when considering the 95% confidence interval of the final fitting (not mentioned in the ASTM E3076, but additionally calculated from the final linear regression of the data in `sdar()` and `sdar_lazy()`), the results of the lazy variant are well within the 95%-CI of the standard variant. There is only a minor difference in the y-range of the final fits resulting in slightly different final results.  
Comparing the regression curves of `sdar_lazy()` and `sdar()` over the mechanical data, you see that there is practically no difference (zoom in ...a lot... to see a difference).
